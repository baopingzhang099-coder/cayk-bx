...
const handleSaveOcr = () => {
  ...
  // Coming from external policy upload flow (OCR + auto-push to clerk)
  if (props.externalPolicyId) {
    const res = store.completePlatformOcrAndPushToClerk(props.externalPolicyId, {
    ...
    MessagePlugin.success('OCR识别完成，已推送跟单员复核')
